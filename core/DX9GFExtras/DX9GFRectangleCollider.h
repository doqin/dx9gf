...
namespace DX9GF {
	...
	public:
		...
		RectangleCollider(std::weak_ptr<TransformManager> transformManager, float width, float height, float x, float y, float rotation = 0, float scaleX = 1, float scaleY = 1) : ICollider(transformManager, x, y, rotation, scaleX, scaleY), width(width), height(height) {}
		RectangleCollider(std::weak_ptr<TransformManager> transformManager, std::weak_ptr<IGameObject> parent, float width, float height, float x, float y, float rotation = 0, float scaleX = 1, float scaleY = 1) : ICollider(transformManager, parent, x, y, rotation, scaleX, scaleY), width(width), height(height) {}
